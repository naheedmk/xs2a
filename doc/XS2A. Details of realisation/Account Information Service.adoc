...
So if any of these fields are present in consent request and ASPSP-Profile contains *accountOwnerInformationSupported=true*, new values store in consent and pass to SPI together with consent object.
...
Get Consent of received consent responds with created consent.
