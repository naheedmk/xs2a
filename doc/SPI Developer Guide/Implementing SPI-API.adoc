...
After the successful authorisation starts (either explicit or implicit), TPP should update the authorisation with data, provided by PSU.
Depending on the amount of SCA methods the PSU has, the amount of PSU data, needed to be provided, differs. In case when PSU
has zero SCA method, only password should be provided. In case when PSU has one SCA method, the password should be provided
