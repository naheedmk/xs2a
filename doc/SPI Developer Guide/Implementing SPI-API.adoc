...
Depending on the amount of SCA methods the PSU has, the amount of PSU data, needed to be provided, differs. In case when PSU
has zero SCA methods, only password should be provided. In case when PSU has one SCA methods, the password should be provided
as well as authentication data(e.g. TAN received be email or SMS). In case when PSU has more than one SCA method,
...
For example, PSU with two SCA methods initiates a payment. The payment was created and the authorisation started. Now TPP should update PSU data
three times:
...
endpoints, the response form ASPSP asks PSU to proceed authorisation in dedicated device. No further authorisation from
TPP is needed. PSU uses the dedicated device and finished authorisation process there.
