...
** "Redirect URI": URI of TPP, where the transaction flow shall be redirected to after a Redirect. Mandated for the *Redirect
SCA Approach* (including OAuth2 SCA approach), specially when TPP-Redirect-preferred equals "true". It is recommended to
always use this header field;
** "Nok redirect URI": if this URI is contained, the TPP is asking to redirect the transaction flow to this address instead
of the TPP-Redirect-URI in case of a negative result of the redirect ScaMethod. This might be ignored by the ASPSP.
...
This interface has 2 implementations: *InitialSpiAspspConsentDataProvider* and *SpiAspspConsentDataProviderImpl*.
First should be used while creating the AIS consent or payment when no ID is present yet. Second should be used while working with
