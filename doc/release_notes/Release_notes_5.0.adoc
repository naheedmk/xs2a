...
== Multilevel SCA for Establish Consent in Redirect approach
From now on, online banking is able to authorised partially AIS Consent by invoking de.adorsys.psd2.consent.service.psu.CmsPsuAisServiceInternal.authorisePartiallyConsent method
or sending request to `POST /psu-api/v1/ais/consent/{consent-id}/authorise-partially-consent` endpoint.
