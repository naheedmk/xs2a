...
public interface CmsAspspAisExportApi {
    Logger log = LoggerFactory.getLogger(CmsAspspAisExportApi.class);
    ...
    default ResponseEntity<Collection<CmsAisAccountConsent>> _getConsentsByTpp(
    ...
    default ResponseEntity<Collection<CmsAisAccountConsent>> getConsentsByTpp(String tppId, LocalDate start, LocalDate end, String psuId, String psuIdType, String psuCorporateId, String psuCorporateIdType, String instanceId) {
        if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
        } else {
            log.warn("ObjectMapper or HttpServletRequest not configured in default CmsAspspAisExportApi interface so no example is generated");
        }
    ...
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "OK")})
    default ResponseEntity<Collection<CmsAisAccountConsent>> _getConsentsByPsu(
        @ApiParam(value = "Creation start date", example = "2010-01-01")
        @RequestHeader(value = "start-date", required = false)
        @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate start,
        @ApiParam(value = "Creation end date", example = "2030-01-01")
    ...
    // Override this method
    default ResponseEntity<Collection<CmsAisAccountConsent>> getConsentsByPsu(LocalDate start, LocalDate end, String psuId, String psuIdType, String psuCorporateId, String psuCorporateIdType, String instanceId) {
        if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
        } else {
            log.warn("ObjectMapper or HttpServletRequest not configured in default CmsAspspAisExportApi interface so no example is generated");
        }
    ...
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "OK")})
    default ResponseEntity<Collection<CmsAisAccountConsent>> _getConsentsByAccount(
        ...
        @ApiParam(value = "ID of the particular service instance")
        @RequestHeader(value = "instance-id", required = false, defaultValue = DEFAULT_SERVICE_INSTANCE_ID) String instanceId) {
        return getConsentsByAccount(aspspAccountId, start, end, instanceId);
    }

    // Override this method
    default ResponseEntity<Collection<CmsAisAccountConsent>> getConsentsByAccount(String aspspAccountId, LocalDate start, LocalDate end, String instanceId) {
        if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
        } else {
            log.warn("ObjectMapper or HttpServletRequest not configured in default CmsAspspAisExportApi interface so no example is generated");
        }
