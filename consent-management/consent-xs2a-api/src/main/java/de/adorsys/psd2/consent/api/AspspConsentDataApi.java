...
public interface AspspConsentDataApi {
    Logger log = LoggerFactory.getLogger(AspspConsentDataApi.class);
    ...
    default ResponseEntity<CmsAspspConsentDataBase64> getAspspConsentData(String encryptedConsentId) {
        if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            ...
            log.warn("ObjectMapper or HttpServletRequest not configured in default AspspConsentDataApi interface so no example is generated");
        }
    ...
    @ApiResponses(value = {
        @ApiResponse(code = 200, message = "OK"),
        @ApiResponse(code = 404, message = "Not Found")})
    default ResponseEntity _updateAspspConsentData(
        @ApiParam(
            name = "consent-id",
            value = "The account consent identification assigned to the created account consent / payment identification assigned to the created payment.",
            example = "CxymMkwtykFtTeQuH1jrcoOyzcqCcwNCt5193Gfn33mqqcAy_xw2KPwMd5y6Xxe1EwE0BTNRHeyM0FI90wh0hA==_=_bS6p6XvTWI",
    ...
    // Override this method
    default ResponseEntity updateAspspConsentData(String encryptedConsentId, CmsAspspConsentDataBase64 request) {
        if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
        } else {
            log.warn("ObjectMapper or HttpServletRequest not configured in default AspspConsentDataApi interface so no example is generated");
        }
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(code = 404, message = "Not Found")})
    default ResponseEntity _deleteAspspConsentData(
        @ApiParam(
            name = "consent-id",
            value = "The account consent identification assigned to the created account consent / payment identification assigned to the created payment.",
            example = "CxymMkwtykFtTeQuH1jrcoOyzcqCcwNCt5193Gfn33mqqcAy_xw2KPwMd5y6Xxe1EwE0BTNRHeyM0FI90wh0hA==_=_bS6p6XvTWI",
    ...
    // Override this method
    default ResponseEntity deleteAspspConsentData(String encryptedConsentId) {
        if (getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
        } else {
            log.warn("ObjectMapper or HttpServletRequest not configured in default AspspConsentDataApi interface so no example is generated");
        }
