...
If parameter = FALSE and "availableAccounts" or "availableAccountsWithBalances" sub-attribute within the "access" attribute has the value "allAccounts" then one-factor authorisation (PSU-ID and password) should be done to set consent status to Valid.

If parameter = TRUE and "availableAccounts" or "availableAccountsWithBalances" sub-attribute within the "access" attribute has the value "allAccounts", then two-factor authorisation(PSU-ID and password + TAN) should be done.|TRUE |TRUE, FALSE
